...
function safeZones(zones) {
  return zones.map(({ _pending, _defenseOptions, ...z }) => ({
    ...z,
...
export function registerSocketEvents(io, db, saveDB) {
  io.on('connection', (socket) => {
    ...
    // OPERATOR: Select a problem (for neutral zone capture)
    socket.on('zone:select_problem', ({ zone_id, problem_id, admin_token }) => {
      if (!auth(admin_token)) return;
      const zone = db.zones.find(z => z.id === zone_id);
      if (!zone || !zone._pending || !zone._pending.options) {
        socket.emit('zone:error', { message: 'No pending selection for this zone' }); return;
      }

      // Find the full problem from the options array stored on _pending
      ...
      // Store the chosen problem on pending
      zone._pending.problem = chosen;
      saveDB();

      // Send the challenge to the zone laptop (hide the answer)
      const { answer: _hidden, ...safeProblem } = chosen;
      socket.emit('zone:challenge_ready', {
        zone_id,
        zone_name: zone.name,
    ...
    socket.on('zone:set_defense_problem', ({ zone_id, team_id, admin_token }) => {
      const isAdmin = auth(admin_token);
      const zone = db.zones.find(z => z.id === zone_id);
      if (!zone || (zone.owner_team_id !== team_id && !isAdmin)) {
        socket.emit('zone:error', { message: 'Only the zone owner can set the defense problem' }); return;
      }
      if (!db.used_zone_problem_ids) db.used_zone_problem_ids = [];

      // Get 5 options — store full objects on zone._defenseOptions so confirm can retrieve them
      const options = getFiveZoneOptions(db.used_zone_problem_ids);
      zone._defenseOptions = options; // temp storage, stripped in fullState()
      saveDB();

      // Send only safe display info to client
      socket.emit('zone:defense_options', {
        zone_id, zone_name: zone.name,
        options: options.map(o => ({ id: o.id, label: o.label, type: o.type })),
      });
    });
    ...
    socket.on('zone:confirm_defense', ({ zone_id, team_id, problem_id }) => {
      ...
      // Look up the full problem from temp _defenseOptions stored during set_defense_problem
      const options = zone._defenseOptions || [];
      const problem = options.find(o => o.id === problem_id);
      if (!problem) { socket.emit('zone:error', { message: 'Problem not found — please try again' }); return; }

      zone.problem = problem;
      zone._defenseOptions = null; // clear temp storage
      saveDB();
